...
make_unique <- function(x, sep = " ", wrap_in_brackets = TRUE, verbose = TRUE){
  ...
  if(!(is.logical(verbose) & length(verbose) == 1)) stop('`verbose` must be a flag, not a ', paste0(class(verbose), collapse = " "))

  if(is.factor(x)) {
    if(verbose){
      warning('make_unique: Converting factor to character variable')
      }
    x <- as.character(x)
  }
